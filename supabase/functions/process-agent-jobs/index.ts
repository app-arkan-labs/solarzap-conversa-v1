...
const markJobCompleted = async (supabase: any, jobId: string) => {
...
const markJobFailed = async (supabase: any, jobId: string, retryCount: number, reason: string) => {
...
const processPostCallJob = async (supabase: any, job: ScheduledAgentJob, lead: LeadRow) => {
  ...
  const { error: invokeError } = await supabase.functions.invoke('ai-pipeline-agent', {
    body: {
      leadId: lead.id,
