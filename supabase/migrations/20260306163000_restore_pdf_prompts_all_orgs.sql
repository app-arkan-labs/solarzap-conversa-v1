-- Migration: 20260306163000_restore_pdf_prompts_all_orgs
-- Restaura default_prompt e agent_goal com os prompts PDF detalhados para TODAS as orgs.
-- Preserva prompt_override (edições manuais do usuário).
-- Condição: só atualiza se default_prompt for curto (<300 chars = placeholder genérico).
...
SET
  ...
  default_prompt = $novo_lead$PROTOCOLO_BASE: PIPELINE_PDF_V1
ETAPA: NOVO_LEAD
...
- Ao receber resposta do lead → marcar objetivo como concluído e mover para "Respondeu".$novo_lead$
WHERE pipeline_stage = 'novo_lead'
  AND (length(coalesce(default_prompt, '')) < 300);

-- 2) RESPONDEU
UPDATE public.ai_stage_config
SET
  agent_goal = 'Qualificar para Chamada Agendada ou Visita Agendada com protocolo BANT minimo obrigatorio.',
  default_prompt = $respondeu$PROTOCOLO_BASE: PIPELINE_PDF_V1
ETAPA: RESPONDEU
...
- Nao fazer multiplas perguntas na mesma mensagem (salvo micro-duplas toleradas em contexto empresarial quando a resposta costuma vir junta).$respondeu$
...
SET
  ...
  default_prompt = $nao_compareceu$PROTOCOLO_BASE: PIPELINE_PDF_V1
...
- address + reference_point (quando visita)$nao_compareceu$
WHERE pipeline_stage = 'nao_compareceu'
  AND (length(coalesce(default_prompt, '')) < 300);

-- 4) PROPOSTA_NEGOCIACAO
UPDATE public.ai_stage_config
SET
  agent_goal = 'Negociar no pos-visita ate compromisso claro de aprovacao ou proximo passo comercial valido.',
  default_prompt = $proposta_negociacao$PROTOCOLO_BASE: PIPELINE_PDF_V1
...
- Nao transformar a negociacao em formulario.$proposta_negociacao$
...
SET
  ...
  default_prompt = $financiamento$PROTOCOLO_BASE: PIPELINE_PDF_V1
ETAPA: FINANCIAMENTO
...
- approved_at
- bank_notes (texto curto)$financiamento$
