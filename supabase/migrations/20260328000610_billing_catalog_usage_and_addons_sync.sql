-- Billing hardening:
-- - map public plans/add-ons to canonical Stripe prices
-- - consume prepaid pack balances correctly over time
-- - return access_state/reason in check_plan_limit
-- - support idempotent sync of recurring add-ons from Stripe subscription items

CREATE UNIQUE INDEX IF NOT EXISTS idx_addon_subscriptions_stripe_item_unique
  ON public.addon_subscriptions (stripe_subscription_item_id);
...
UPDATE public._admin_addon_catalog
SET
  stripe_price_id = CASE addon_key
    WHEN 'whatsapp_extra' THEN 'price_1TG7CRDgShBNMYMj0EsfjD3t'
    WHEN 'automations_10k' THEN 'price_1TG7CSDgShBNMYMj1XoZ19On'
    WHEN 'ai_pack_1k' THEN 'price_1TG7CTDgShBNMYMjgV7LXX3L'
    WHEN 'ai_pack_5k' THEN 'price_1TG7CVDgShBNMYMjADTu00qb'
    WHEN 'ai_pack_20k' THEN 'price_1TG7CdDgShBNMYMjxiM0QfX8'
    WHEN 'disparo_pack_1k' THEN 'price_1TG7CWDgShBNMYMjzaQawos6'
    WHEN 'disparo_pack_5k' THEN 'price_1TG7CXDgShBNMYMj9D05fl0o'
    WHEN 'disparo_pack_25k' THEN 'price_1TG7CZDgShBNMYMjAqsaeXnX'
    ELSE stripe_price_id
  END
WHERE addon_key IN (
  'whatsapp_extra',
...
CREATE OR REPLACE FUNCTION public.billing_limit_key_from_event(p_event_type text)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
  CASE p_event_type
    WHEN 'broadcast_credit_consumed' THEN RETURN 'monthly_broadcast_credits';
    WHEN 'ai_request' THEN RETURN 'included_ai_requests_month';
    WHEN 'automation_execution' THEN RETURN 'max_automations_month';
    ELSE RETURN NULL;
  END CASE;
END;
$$;
...
DECLARE
  ...
  v_plan_max integer := 0;
  v_recurring_increment integer := 0;
  v_previous_counter integer := 0;
  v_pack_spent integer := 0;
  v_available_pack integer := 0;
BEGIN
  ...
  SELECT COALESCE(o.plan_limits, '{}'::jsonb)
    INTO v_plan_limits
  ...
  v_limit_key := public.billing_limit_key_from_event(p_event_type);
  v_credit_type := public.billing_credit_type_from_limit(v_limit_key);

  IF v_limit_key IS NOT NULL THEN
    v_plan_max := COALESCE((v_plan_limits ->> v_limit_key)::integer, 0);
    ...
    WHERE a.org_id = p_org_id
      ...
      AND c.limit_key = v_limit_key;

    SELECT COALESCE(MAX(uc.value), 0)
      INTO v_previous_counter
    FROM public.usage_counters uc
    WHERE uc.org_id = p_org_id
      AND uc.billing_cycle = v_cycle
      AND uc.counter_key = v_counter_key;

    IF v_credit_type IS NOT NULL AND v_plan_max <> -1 THEN
      v_pack_spent :=
        GREATEST(
          0,
          (v_previous_counter + p_quantity)
          - GREATEST(COALESCE(v_plan_max, 0), 0)
      ...
      IF v_pack_spent > 0 THEN
        SELECT COALESCE(MAX(cb.balance), 0)
          INTO v_available_pack
        FROM public.credit_balances cb
        WHERE cb.org_id = p_org_id
          AND cb.credit_type = v_credit_type;

        IF v_available_pack < v_pack_spent THEN
  ...
  IF v_pack_spent > 0 AND v_credit_type IS NOT NULL THEN
    UPDATE public.credit_balances
    SET balance = balance - v_pack_spent,
        updated_at = now()
  ...
  RETURN jsonb_build_object(
    ...
    'limit_key', v_limit_key,
    'credit_type', v_credit_type,
    'pack_spent', v_pack_spent
  );
END;
$$;
...
BEGIN
  ...
  v_plan_limits := COALESCE(v_org.plan_limits, '{}'::jsonb);
  v_plan_max := COALESCE((v_plan_limits ->> p_limit_key)::integer, 0);

  IF COALESCE(v_org.plan, '') = 'unlimited' THEN
    v_access_state := 'full';
  ELSE
  ...
  IF v_plan_max = -1 THEN
    ...
    v_allowed := v_access_state = 'full';
    v_reason := CASE
      WHEN v_access_state = 'read_only' THEN 'subscription_read_only'
      WHEN v_access_state = 'blocked' THEN 'subscription_blocked'
      ELSE null
    END;
  ...
  v_projected := v_current + p_quantity;
  v_allowed := v_access_state = 'full' AND v_projected <= v_effective_max;
  v_reason := CASE
    WHEN v_access_state = 'read_only' THEN 'subscription_read_only'
    WHEN v_access_state = 'blocked' THEN 'subscription_blocked'
