...
export function useOnboardingProgress(enabled = true) {
  ...
  const query = useQuery({
    queryKey,
    enabled: enabled && Boolean(user?.id && orgId),
    staleTime: 15_000,
    queryFn: async (): Promise<OnboardingProgress | null> => {
      ...
      const { data, error } = await supabase
        ...
        .select('user_id, org_id, current_step, completed_steps, skipped_steps, tour_completed_tabs, is_complete')
        .eq('user_id', user.id)
        .eq('org_id', orgId)
        .maybeSingle();
  ...
  const patchMutation = useMutation({
    mutationFn: async (patch: ProgressPatch): Promise<OnboardingProgress | null> => {
      if (!user?.id || !orgId) return null;
      const targetOrgId = patch.org_id ?? orgId;
